...
public class BstSetKTU<E extends Comparable<E>> implements SortedSetADT<E>, Cloneable {
    ...
    public boolean addAll(BstSetKTU<? extends E> items) {
    ...
    @Override
    public SortedSetADT<E> subSet(E element1, boolean fromInclusive, E element2, boolean toInclusive)
    {
        BstSetKTU<E> newSet = new BstSetKTU<E>();
        Iterator<E> j = this.iterator();
        E startingElement = null;
        E elements = null;
        while(startingElement != element1){ // from element 1
            startingElement = j.next();
        }
        if(startingElement == element1 && !fromInclusive) 
        {
            startingElement = j.next();
        }
       while(elements != element2)
       {
        elements = j.next();
        newSet.add(elements);
       }
    ...
    public boolean add(E element) {
        ...
        root = addRecursive(element, (BstNode<E>) root);
        if (!returned) {
            returned = true;
            return false;
        }
    ...
    public SetADT<E> subSet(E element1, E element2) {
        ...
        do{ // this will be true until g will be euqals last element
            elements = j.next();
            newSet.add(elements);
        }while(elements != element2);
    ...
    public E lower(E e) {
        ...
        if(c.compare(e, node.element) > 0) node = node.right;
        else node = node.left;
        
        while(c.compare(e, node.element) > 0)
        {
           
        if(node.right != null && c.compare(node.element, node.right.element) < 0 && c.compare(e, node.right.element) > 0)
        {
            node = node.right;
        }
        else if(node.right != null && node.right.left != null && c.compare(e, node.right.element) < 0 && c.compare(node.element, node.right.left.element) < 0 && c.compare(e, node.right.left.element) > 0 )
        {
            node = node.right.left;
        }
        else if(node.left != null && c.compare(e, node.left.element) > 0 && c.compare(node.element, node.left.element) < 0) 
        {
            node = node.left;
        }
        else
