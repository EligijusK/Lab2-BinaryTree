...
public class GreitaveikosTyrimas {
    ...
    private final SortedSetADTx<Game> aSeries = new BstSetKTUx(new Game(), Game.byPrice);
    private final SortedSetADTx<Game> aSeries2 = new BstSetKTUx2(new Game());
    private final SortedSetADTx<Game> aSeries3 = new AvlSetKTUx(new Game());
    ...
    public void SisteminisTyrimas() throws InterruptedException {
        try {
            for (int k : TIRIAMI_KIEKIAI) {
                Game[] autoMas = GameCreating.generateAndMix(k, 1.0);
                aSeries.clear();
                aSeries2.clear();
                aSeries3.clear();
                tk.startAfterPause();
